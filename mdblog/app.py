...
def add_article() :
    ...
    if add_form.validate():
        ...
        db.session.commit()
        flash("Article was saved..", "list-group-item list-group-item-success")
        return redirect(url_for("view_articles"))
    else:
        for error in add_form.errors:
            flash("{} is required".format(error), "list-group-item list-group-item-danger")
...
def edit_article(art_id):
    ...
    if article:
        ...
        if edit_form.validate():
            ...
            db.session.commit()
            flash("Edit saved.", "list-group-item list-group-item-success")
            return redirect(url_for("view_article", art_id=art_id))
        else:
            for error in login_form.errors:
                flash("{} is missing".format(error), "list-group-item list-group-item-danger")
                return redirect(url_for("view_login"))
...
def change_password():
    ...
    if form.validate():
        ...
        if user and user.check_password(form.old_password.data):
            ...
            db.session.add(user)
            db.session.commit()
            flash("Password changed!", "list-group-item list-group-item-success")
            return redirect(url_for("view_admin"))
        else:
            flash("Invalid credentials", "list-group-item list-group-item-danger")
            return render_template("change_password.html", form=form)
